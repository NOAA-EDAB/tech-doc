...
**Public availability statement**: NEFSC survey data used in these analyses are available upon request (see [BTS metadata](https://inport.nmfs.noaa.gov/inport/item/22560) for access procedures). Derived condition data are available [here](https://comet.nefsc.noaa.gov/erddap/tabledap/gf_condition_2018.html).
...
### Data analysis
The following growth curve was fit through individual fish lengths and weights from the NEFSC bottom trawl survey data from 1992-20XX to produce reference length-weight parameters:
...
```{r condition-factor, eval = T, fig.width = 8, fig.cap = "Normalized condition factors of MAFMC managed species.", message=F, warning=F}
...
#Get data
CF <- read.csv(file.path(data.dir, "MAFMC_RelativeWeight_2018.csv"))

#Processing
CF_mat <- CF %>%
  unite(.,Var,c("species","stock","sex"), sep =", ") %>% #Unite groupings
  gather(.,Time,Value,-Var) %>% #Convert to long
  mutate(Time = as.numeric(gsub("X","",Time))) %>% #Tidy years 
  group_by(Var) %>% 
  mutate(Value = ztrans(Value)) %>% #Normalize
  spread(.,Time,Value) %>% #Convert to wide for plotting
  arrange(Var) 
...
image(z = t(as.matrix(CF_mat[,2:27])),
...
mtext(text = row_names,side = 4, las = 1,
      cex = 0.7, at = seq(0,1,0.071),
      line = 1)

#Figure key
par(mar = c(0.7, 0.5, 0.2, 0.5),
    fig = c(0.2, 0.5, 0, 0.1),
    new = T)

...
#combine factors and organize
rows <- paste(CF$species,CF$stock,CF$sex)
rows <- tolower(rows) %>% capitalize() 
CF <- CF %>% select(-c(species, stock, sex))
names(CF) <- gsub("X","",names(CF))

#standardize data
CF <- as.data.frame(apply(CF,1,ztrans))

#set rowname and transpose for d3heatmap
CF <- CF %>% `names<-`(rows)
...
# axis(4, at = -, 
#      label = paste(names(CF), 
#                    names(CF)), las = T, lwd = 0)

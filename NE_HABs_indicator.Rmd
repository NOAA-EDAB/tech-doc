...
```{r closuresrc, echo = F, include = T, results='asis', eval = F}
tabl <- "
...
"
#cat(tabl)
df<-readr::read_delim(tabl, delim="|")
df<-df[-c(1,2) ,c("State","Source Organization")]
knitr::kable(
  df, booktabs = TRUE,
  caption = 'Closure information providers.'
)
...
```{r packages, echo=T, message=FALSE, warning=FALSE, include=T, eval = F}
...
```{r NE_HAB, echo=T,message=FALSE, warning=FALSE , eval = F}
...
```{r NE-HAB-image, echo = F, fig.align="center",fig.cap="Regional HAB-related shellfish bed closures in New England (2007-2016)."}
<<<<<<< HEAD
image.dir <- 'images'
=======
image.dir <- './images'
>>>>>>> 97e8e2593b8bce39d0da95166e1bc52fd2183bbc
knitr::include_graphics(file.path(image.dir, 'NE_HABs.png'))
```


### ERDDAP data set
Data are available [here](http://comet.nefsc.noaa.gov/erddap/tabledap/NE_hab_2018.htmlTable?a,longitude,latitude,val,var,index).
```{r erddap pull, echo = F, eval = F}
library(rerddap)

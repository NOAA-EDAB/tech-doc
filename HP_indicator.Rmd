...
```{r harbor-porpoise, fig.cap="Harbor porpoise bycatch estimated shown with Potential Biological Removal (red) and confidence intervals (orange).", echo = F, message=F, warning=F, fig.pos='H', fig.height=4}
...
lw_CI <- SOE.data.2018[Var == 'Harbor porpoise bycatch 2.5 CI',
                       list(Time, Value)]
up_CI <- SOE.data.2018[Var == 'Harbor porpoise bycatch 97.5 CI',
                       list(Time, Value)]
pbi   <- SOE.data.2018[Var == 'Harbor porpoise potential biological removal',
                       list(Time, Value)]

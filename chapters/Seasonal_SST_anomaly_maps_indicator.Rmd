...
### Plotting
```{r sst-anomaly-maps, fig.cap="Sea surface temperature anomalies in winter (A), spring (B), summer (C), and fall (D) 2017. Black lines depict the 200 m isobath." , fig.width = 8,fig.height = 5.5, echo=T, message=F, warning=F}
...
SST_plot <- function(raster_data, text,pos = "left"){
  ...
  d <- raster::raster(file.path(gis.dir,raster_data))
  raster::crs(d) <- map.crs
  #crop data
  d <- raster::crop(d, e)
  ...
  #plot raster
  raster::plot(d, col=colors(n=color_levels),
  ...
  #plot contour
  raster::contour(bathy,drawlabels = T,add=T, nlevels = 2, zlim = c(-200,-200))
  #plot coastline

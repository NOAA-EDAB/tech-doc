...
Common tern chick diet is quantified at each of the seven nesting sites by observing chick provisioning from portable observation blinds. The locations of observation blinds within each site are chosen to maximize the number of visible nests, and provisioning observations take place between mid-June and early August annually. Observations of chick diet are made during one or two three to four hour periods throughout the day, but typically proceed according to nest activity levels (moreso in the morning hours). Observations begin with chicks as soon as they hatch, and continue until the chicks fledge or die. 

Most common tern prey species are identifiable to the species level due to distinct size, color and shape. However, when identification is not possible or is unclear, prey species are listed as "unknown" or "unknown fish". More detailed methods can be found in @hall2000. 

**Nest productivity**

Common tern nest productivity, in terms of the number of fledged chicks per nest, is collected annually from fenced enclosures at each study site ("productivity plots"). Newly hatched chicks within these enclosures are weighed, marked or banded, and observed until fledging, death, or until a 15 day period has passed when chicks are assumed to have fledged. Productivity is also quantified from observer blinds for nests outside of the productivity plots where chicks are marked for identification. More detailed methods for quantifying nest productivity can be found in @hall2004
...
```{r map-and-prey, fig.cap = "Common terns: a. Locations of the seven sampled common tern nesting sites in Gulf of Maine (EER = Eastern Egg Rock, JI = Jenny Island, MR = Matinicus Rock, OGI = Outer Green Island, PINWR = Pond Island National Wildlife Refuge, SINWR = Seal Island National Wildlife Refuge, STI = Stratton Island), and b. Prey frequencies in the diets of common tern observed across the seven colonies in Gulf of Maine. Prey occurring in <5\\% of common tern diets were excluded for clarity.", fig.height = 4, fig.width = 8}

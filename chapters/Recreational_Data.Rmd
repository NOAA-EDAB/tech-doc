...
```{r, code = readLines(here::here("source/get_rec.R")), eval = F, echo = T}

```

### Plotting

```{r rec-div, fig.cap = "Recreational effort diversity and diversity of recreational catch in New England."}
...
```{r rec-landings-ne, fig.cap = "Total recreational seafood harvest in New England."}
...
```{r rec-op, fig.cap = "Recreational effort and number of recreational anglers in New England."}
recdat <- ecodata::recdat %>% 
  filter(EPU == "NE") %>% 
  group_by(Var) %>% 
  mutate(hline = mean(Value))
